...
from floras_helpers.binning import bin_spikes_pos_and_time, bin_mua_per_depth,get_binned_rasters
...
# to one dimension. # so flattened gaussian
...
class rf_model(): 
    ...
    """
    def __init__(self,ev,spikes): # maybe sparseNoise needs to go from init
        """
        initiate screen parameters and parameters for binning rasters & take data input in the correct format
        ev: Bunch/pd.DataFrame
            events from PinkRigs. Needs to contail at least squareAzimuth,squareElevation,squareOnTimes
        spikes: Bunch
            spikes from PinkRigs. Needs to contain at least times and clusters
        ...
        assert hasattr(ev,'squareAzimuth'), "events need to contain squareAzimuth"
        assert hasattr(ev,'squareElevation'), "events need to contain squareElevation"
        assert hasattr(ev,'squareOnTimes'), "events need to contain squareOnTimes"
        assert hasattr(spikes,'times'), "spikes need to contain times"
        assert hasattr(spikes,'clusters'), "spikes need to contain clusters"

        self.format_events(ev)
        self.spikes = spikes
    ...
    def get_response_stim_triggered(self,mode='per-neuron',depth_spacing = 60,shuffle_seed=None,selected_ids=None,delay_for_vistrig=0.02,cv_split = 2):
        """
        ...
        depth_spacing: float
            spacing of depth bins for mua calculation (in um) only used if mode='per-depth'
        ...
        if mode=='per-neuron':
            ...
            spikes_depths = bin_mua_per_depth(self.spikes,depth_spacing=depth_spacing)            
    ...
    def plot_fit(self,ID='0',**response_kwargs):
        ...
        fig,ax = plt.subplots(len(ID),self.responses.cv_number.size+1,sharex=True,sharey=True,figsize=(10,len(ID)*3))        
        if ax.ndim==1:
            ax = ax[np.newaxis,:]
        # plot the prediction
