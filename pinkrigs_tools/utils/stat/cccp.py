...
from ..ev_utils import parse_av_events


def get_mann_whitneyU(x,y,n_shuffles=20):
    """
    function to calculate the mann-Whitney U(x) statistic where we compare the x with the y distribution
    Parameters .e.g.
    x = Right choice
    y = left choice
...
class cccp():
    """
    wrapper around ev structures to perform efficient cccp for PinkRigs data
    """
    def __init__(self):
    ...
    def load_and_format_data(self,rec=None,**kwargs):
        ...
        if rec is None:
            recordings  = load_data(
                ...
                merge_probes = True, 
                cam_hierarchy = ['sideCam','frontCam','eyeCam'],**kwargs
            )
            rec = recordings.iloc[0]
        
        
        self.spikes,self.clusters,self.cam = (rec.probe.spikes,
                                                rec.probe.clusters,
                                                rec.camera)
        ...
        self.ev,self.trial_types = parse_av_events(
